...
const MultiSelectorTrigger = forwardRef<
  HTMLButtonElement,
  TriggerProps & { options?: MultiSelectOptionsType }
>(
  (
    {
    ...
    return (
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn(
            "text-muted-foreground border-brand flex h-auto min-h-9 w-full flex-wrap justify-start gap-1 border p-1 px-4",
...
const MultiSelectorList = forwardRef<HTMLDivElement, ListProps>(
  ({ className, children, itemsCount }, ref) => {
    ...
    const selectableChildren = filteredChildren.filter(
      (child: any) => !child.props?.disabled
    );

    const allSelectableSelected =
      selectableChildren.length > 0 &&
      selectableChildren.every((child: any) =>
      ...
      if (allSelectableSelected) {
        selectableChildren.forEach((child: any) => {
          if (values.includes(child.props.value))
            dispatch({ type: "TOGGLE_VALUE", payload: child.props.value });
        });
      } else {
        selectableChildren.forEach((child: any) => {
          if (!values.includes(child.props.value))
...
>(({ children, ...props }, ref) => {
  ...
  const isInput = (c: React.ReactElement) =>
    c &&
    (c.type === MultiSelectorInput ||
      (c.type as any)?.displayName === "MultiSelectorInput");
  const isList = (c: React.ReactElement) =>
    c &&
    (c.type === MultiSelectorList ||
      (c.type as any)?.displayName === "MultiSelectorList");

  const inputChild = childrenArray.find(isInput) ?? null;
  const listChild = childrenArray.find(isList) ?? null;
  const otherChildren = childrenArray.filter(
    (c) => c !== inputChild && c !== listChild
  );
